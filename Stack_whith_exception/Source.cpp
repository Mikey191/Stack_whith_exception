...
//если баланса нет выдать исключение с помощью try catch
//так же вызвать исключение, если входная строка имеет больше 50 символов
...
void MaxSymb(string str)
{
	try
	{
		if (str.length() > 50)
			throw "Error. String length more 50 symbols!";
	}
	catch (const char* c)
	{
		cout << c << endl;
...
int main()
{
	try
	{
		string str{ "(asdf)(asdfs)[dssfe]876427475476542e3ygdedhgvcregvyytr3tr3y4763r3" };
		MaxSymb(str);//проверка на максимальное кол-во символов
		Balance(str);//проверка на баланс скобок
	}
	catch (...)
	{
		cout << "Error!";
...
void Balance(string str)
{
	...
	{
		cout << "String isn't have balance "<< c << endl;
	}
}
